...
1. grad(theta) = -grad(phi)*exp(-theta) + grad(s)
2. -lap(theta) - k_const*[(3-b)*grad(theta)-(1+a)*grad(Y)] = (ε/K)*exp(-theta)-(grad(theta))^2
3. (gamma-1)*Y + (sc/cp)*gamma*s = theta

Usage:
    structure_kramers_theta.py [options]
...
def kramers_opacity_polytrope(nz, γ, ε, n_h, verbose=False, dealias=2,
    ...
    print(np.shape((lap(θ0)).evaluate()['g']))
    print(np.shape(((grad(θ0)@ez).evaluate()['g'])))
    print(np.shape((grad(θ0)@grad(θ0)).evaluate()['g']))
    print(ε)

    problem = de.NLBVP([s0, Υ0, θ0, τ_s1, τ_s2, τ_h1])
    problem.add_equation((grad(θ0) - s_c_over_c_P*grad(s0) + lift(τ_h1,-1),
                         -grad_φ*ez*np.exp(-θ0)))
    problem.add_equation((-lap(θ0) + lift(τ_s1,-1) + lift(τ_s2,-2), κ_const*grad(θ0)@((3-bb)*grad(θ0)-(aa+1)*grad(Υ0)) +
                          ε*np.exp(-θ0)/κ0 - grad(θ0)@grad(θ0)))
    problem.add_equation(((γ-1)*Υ0 + s_c_over_c_P*γ*s0 - θ0, 0))
    problem.add_equation((θ0(z=0), np.log(h_bot)))
    problem.add_equation((θ0(z=Lz), np.log(1.3)))
    ...
    if verbose:
        ...
        plt.subplot(1,5,1)
        plt.text(-0.45, 1.59, '(c)', fontsize=15)
        ...
        plt.savefig('kramers_solve_theta_linear_ic2_nh{}_eps{:.3g}_gamma{:.3g}.pdf'.format(n_h,ε,γ),bbox_inches='tight')
