...
#    κ00=10000000.0
    ...
    problem.add_equation((grad(θ0) + lift(τ_h1,-1),
                         -np.exp(-θ0)*grad_φ*ez + s_c_over_c_P*grad(s0)))
    ...
    h0['g'] = np.exp(θ0).evaluate()['g']
    κ0['g'] = (κ_const*(np.exp(θ0))**(3-bb)/(np.exp(Υ0))**(1+aa)).evaluate()['g']
